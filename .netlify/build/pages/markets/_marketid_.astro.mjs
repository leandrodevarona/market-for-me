...
/* empty css                                         */
import 'clsx';
import { $ as $$ProductCard } from '../../chunks/CartModal_BShAwnfr.mjs';
import { $ as $$Layout } from '../../chunks/Layout_CxbHblSA.mjs';
import { g as getMarketById } from '../../chunks/markets_BsxLjTlS.mjs';
...
const $$MarketProductList = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${(!products || products.length <= 0) && renderTemplate`${maybeRenderHead()}<div>No products</div>`}<ul>${products.map((product) => renderTemplate`${renderComponent($$result, "ProductCard", $$ProductCard, { "product": product })}`)}</ul>`;
